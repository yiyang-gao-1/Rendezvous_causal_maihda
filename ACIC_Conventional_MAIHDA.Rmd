...
- **Intersectional strata formation**
  - Created by crossing: sex × risk × comorbidity × deprivation × plan
  
- **Between-stratum variation quantification**
  - Measured using VPC (Variance Partition Coefficient)
  
- **Treatment heterogeneity modeling**
  - Implemented through random slope of Wc

```{r load-merge}
# Choose a replicate (feel free to change this integer for other replicates)
replicate_id_int <- 52                                   # replicate number as integer
...
# if receiving warning messages about "isSingular"
...
# Pull variance components as a data frame from Model B
vcB_df <- as.data.frame(VarCorr(mB))

# Helper: get diagonal variance for a given group/term (var2 must be NA on the diagonal)
get_var <- function(vdf, group, term) {
  out <- vdf$vcov[vdf$grp == group & vdf$var1 == term & is.na(vdf$var2)]
  if (length(out) == 0) NA_real_ else out[1]
}

# Model B: variances
v_strata_B <- get_var(vcB_df, "strata_label", "(Intercept)")   # between-stratum variance
v_practice <- get_var(vcB_df, "id.practice",  "(Intercept)")   # practice variance
v_patient  <- get_var(vcB_df, "id.patient",   "(Intercept)")   # patient variance
v_resid_B  <- tryCatch(as.numeric(sigma(mB))^2, error = function(e) NA_real_)

# VPC (denominator includes all random parts)
...
# Tutorial (canonical) VPC (denominator = strata + residual only)
VPC_strata_canonical <- if (all(is.finite(c(v_strata_B, v_resid_B))) &&
                            (v_strata_B + v_resid_B) > 0) {
  v_strata_B / (v_strata_B + v_resid_B)
} else NA_real_

# Model C: slope heterogeneity
vcC_df <- as.data.frame(VarCorr(mC))
sd_W   <- sqrt(get_var(vcC_df, "strata_label", "Wc"))          # may be NA if no Wc term
sd_int <- sqrt(get_var(vcC_df, "strata_label", "(Intercept)")) # NA if slope-only model

# Intercept–slope covariance exists only when both terms are present
cov_iw <- vcC_df$vcov[vcC_df$grp == "strata_label" &
                      vcC_df$var1 == "(Intercept)" &
                      vcC_df$var2 == "Wc"]
cov_iw <- if (length(cov_iw)) cov_iw[1] else NA_real_

# Correlation only if both variances are positive and finite
corr_iw <- if (is.finite(sd_W) && is.finite(sd_int) && sd_W > 0 && sd_int > 0) {
  cov_iw / (sd_W * sd_int)
} else NA_real_
...
knitr::kable(
  tibble::tibble(
    quantity = c("VPC (strata and residual only)",
                 "VPC (all random parts)"),
...
# --- Lightweight caterpillars: sample a subset, no CI ------------

MAX_STRATA_B   <- 50    # how many strata to show for Model B (intercepts)
MAX_STRATA_C   <- 50    # how many strata to show for Model C (slopes)
SELECT_METHOD  <- "top"    # "top" = largest by size, "random" = random sample
SEED_FOR_PLOTS <- 2025     # set a seed for reproducible sampling

# Pre-compute stratum sizes (used to pick "top" strata)
sizes <- df %>% dplyr::count(strata_label, name = "n")

# Helper: choose a subset of rows by "top" or "random"
choose_subset <- function(dat, n_show, method = c("top","random")) {
  method <- match.arg(method)
  if (nrow(dat) <= n_show) return(dat)
  if (method == "top") {
    dat %>% dplyr::slice_max(order_by = n, n = n_show, with_ties = FALSE)
  } else {
    set.seed(SEED_FOR_PLOTS)
    dat %>% dplyr::slice_sample(n = n_show)
  }
}

# ---------------- Model B: strata random intercept caterpillar (no CI) --------
reB_list <- try(lme4::ranef(mB, condVar = FALSE), silent = TRUE)

if (!inherits(reB_list, "try-error") && "strata_label" %in% names(reB_list)) {
  reB <- reB_list$strata_label
  cat_df_B <- tibble::tibble(
    stratum = rownames(reB),
    effect  = reB[, "(Intercept)"]
  ) %>%
    dplyr::left_join(sizes, by = dplyr::join_by(stratum == strata_label)) %>%
    dplyr::mutate(n = n %||% 0L)

  # pick subset
  cat_df_B_sub <- choose_subset(cat_df_B, MAX_STRATA_B, method = SELECT_METHOD) %>%
    dplyr::arrange(effect) %>%
    dplyr::mutate(idx = dplyr::row_number())

  # plot
  ggplot(cat_df_B_sub, aes(x = idx, y = effect)) +
    geom_hline(yintercept = 0, linetype = 3) +
    geom_point(size = 0.45) +
    labs(
      title    = "Caterpillar: strata random intercepts (Model B)",
      subtitle = paste0("Showing ", nrow(cat_df_B_sub), " of ", nrow(cat_df_B), " strata (", SELECT_METHOD, ")"),
      x = "Strata (ordered subset)", y = "BLUP (centred)"
    ) +
    theme_minimal()
} else {
  cat("Note: Model B random intercepts for strata not available; skipped.\n")
}

# ---------------- Model C: strata random slope for Wc (no CI) -----------------
reC_list <- try(lme4::ranef(mC, condVar = FALSE), silent = TRUE)
has_strata <- !(inherits(reC_list, "try-error")) && ("strata_label" %in% names(reC_list))

if (has_strata) {
  reC <- reC_list$strata_label

  # column name for the slope (allow either 'Wc' or 'W' depending on your final spec)
  slope_col <- intersect(colnames(reC), c("Wc","W"))
  if (length(slope_col) == 1) {
    slope_col <- slope_col[1]
    ...
    # pick subset
    catW_sub <- choose_subset(catW, MAX_STRATA_C, method = SELECT_METHOD) %>%
      dplyr::arrange(slope) %>%
      dplyr::mutate(idx = dplyr::row_number())

    ggplot(catW_sub, aes(x = idx, y = slope)) +
      geom_hline(yintercept = 0, linetype = 3) +
      geom_point(size = 0.45) +
      labs(
        title    = paste0("Caterpillar: strata random slope for ", slope_col, " (Model C)"),
        subtitle = paste0("Showing ", nrow(catW_sub), " of ", nrow(catW), " strata (", SELECT_METHOD, ")"),
        x = "Strata (ordered subset)", y = paste0("Deviation from overall ", slope_col, " effect")
      ) +
      theme_minimal()
  } else {
    cat("Note: Model C does not contain a 'Wc' (or 'W') random slope for strata; slope caterpillar skipped.\n")
  }
} else {
  cat("Note: Model C random effects for strata not available; skipped.\n")
}
