...
# Notes:
# - CI calculation needs condVar=TRUE, which can be heavy for thousands of strata.
# - We auto-disable CIs when groups are large (thresholds below).
# - Figures are saved as PNG via knitr's device to keep memory down.

# thresholds you can tune
th_ci_strata  <- 800   # if number of strata <= this, compute CIs for intercepts
th_ci_slope   <- 400   # if number of strata <= this, compute CIs for slopes

J_strata <- nlevels(df$strata_label)

# ---------- Model B: strata random intercept caterpillar ----------
use_ci_B <- J_strata <= th_ci_strata

if (use_ci_B) {
  rrB <- lme4::ranef(mB, condVar = TRUE)
  reB <- rrB$strata_label
  pvB <- attr(rrB$strata_label, "postVar")  # 1 x 1 x J array for random intercept only
  seB <- sqrt(sapply(1:dim(pvB)[3], function(j) pvB[,,j]))
  ...
  ggplot(cat_df_B, aes(x = idx, y = effect)) +
    geom_hline(yintercept = 0, linetype = 3) +
    geom_errorbar(aes(ymin = ciL, ymax = ciU), width = 0) +
    geom_point(size = 0.6) +
    labs(title = "Caterpillar: strata random intercepts (Model B)",
         subtitle = "95% CIs shown (few strata → full CI calculation)",
         x = "Strata (ordered)", y = "BLUP (centred)") +
    theme_minimal()
} else {
  # fast path: no CI
  reB <- lme4::ranef(mB, condVar = FALSE)$strata_label
  cat_df_B <- tibble::tibble(
    stratum = rownames(reB),
    effect  = reB[,"(Intercept)"]
  ) %>% arrange(effect) %>% mutate(idx = row_number())
  ...
  ggplot(cat_df_B, aes(x = idx, y = effect)) +
    ...
    geom_point(size = 0.4) +
    labs(title = "Caterpillar: strata random intercepts (Model B)",
         subtitle = "CIs omitted for speed (many strata)",
         x = "Strata (ordered)", y = "BLUP (centred)") +
    theme_minimal()
}

# ---------- Model C: strata random slope for Wc ----------
# We first check whether the fitted mC actually has a slope term named "Wc".
reC_list <- try(lme4::ranef(mC), silent = TRUE)
has_Wc <- !(inherits(reC_list, "try-error")) &&
          ("strata_label" %in% names(reC_list)) &&
          any(colnames(reC_list$strata_label) == "Wc")

if (has_Wc) {
  use_ci_C <- (J_strata <= th_ci_slope)

  if (use_ci_C) {
    rrC <- lme4::ranef(mC, condVar = TRUE)
    reC <- rrC$strata_label
    pvC <- attr(rrC$strata_label, "postVar")     # 2 x 2 x J or 1 x 1 x J depending on RE structure
    ...
    # fast path: no CI
    reC <- lme4::ranef(mC, condVar = FALSE)$strata_label
    catW <- tibble::tibble(
      stratum = rownames(reC),
      slope   = reC[,"Wc"]
    ) %>% arrange(slope) %>% mutate(idx = row_number())
  ...
  # If no Wc random slope (e.g., slope-only model not present or named differently), skip politely
  cat("Note: Model C does not contain a 'Wc' random slope for strata; slope caterpillar skipped.\n")
}
